...
let ALLOWED_NAMES = ['bunny chan','bunnychan','bunny'];
let LEVEL1_HINT = '💡 indice : le petit surnom que je te donne tout le temps... ça commence par un "b" 🐰';
...
const QUIZ_QUESTIONS = [
  "Si notre amour avait une odeur, ce serait laquelle ?",
  "Où dans le monde tu t'enfuirais avec moi demain ?",
  "Le petit détail chez moi qui te fait toujours sourire ?",
  "Notre chanson, en trois mots ?",
  "Une promesse que tu veux de moi, pour toujours ?",
];
...
const INSTRUCTIONS = {
  1: {
    title: 'Mot Secret',
    body: `
      <p>Un petit quiz du surnom secret — dis-le comme moi seul je le dis ♥</p>
      <ul>
        <li>Tape le petit surnom que je te donne, dans la case</li>
        <li>Tu as <strong>3 essais</strong> — ton cœur connaît la réponse</li>
        <li>Appuie sur <strong>Dis-moi</strong> ou sur Entrée pour valider</li>
      </ul>
    `
  },
  2: {
    title: 'Souvenirs à Deux',
    body: `
      <p>Un jeu de cartes à retourner — chaque paire trouvée réveille un souvenir de nous ✿</p>
      <ul>
        <li>Touche une carte pour la retourner et révéler son symbole</li>
        <li>Touche une deuxième carte — si les symboles correspondent, elles restent retournées</li>
        <li>Si elles ne correspondent pas, elles se retournent. Retiens bien où elles étaient !</li>
        <li>Trouve les <strong>8 paires</strong> pour gagner</li>
        <li>Essaie de finir en le moins de <strong>coups</strong> possible</li>
      </ul>
    `
  },
  3: {
    title: 'Puzzle Photo',
    body: `
      <p>Une photo de nous en mille morceaux — glisse chaque pièce à sa bonne place ✿</p>
      <ul>
        <li><strong>Glisse</strong> chaque pièce avec ta souris ou ton doigt</li>
        <li>Quand tu déposes une pièce <strong>près de son bon endroit</strong>, elle s'emboîte toute seule</li>
        <li>Les pièces bien placées ont une petite <strong>lueur verte</strong> et se verrouillent</li>
        <li>Utilise le <strong>modèle</strong> sur le côté comme repère</li>
        <li>Touche <strong>↻ mélanger</strong> si tu veux recommencer</li>
      </ul>
    `
  },
  4: {
    title: 'Pluie de Cœurs',
    body: `
      <p>Attrape l'amour qui tombe du ciel — remplis mon cœur jusqu'en haut ♥</p>
      <ul>
        <li>Touche les <strong>cœurs roses</strong> — chacun remplit mon cœur</li>
        <li><strong>Évite</strong> les cœurs sombres ✗ — ils ne comptent pas</li>
        <li>Tu as <strong>35 secondes</strong> pour attraper <strong>15 cœurs</strong></li>
        <li>Les cœurs tombent doucement et sont bien gros — faciles à toucher ✿</li>
      </ul>
    `
  },
  5: {
    title: 'Constellation d\'Amour',
    body: `
      <p>Relie les étoiles qui brillent, dans l'ordre, pour dessiner notre forme ✦</p>
      <ul>
        <li>Touche d'abord l'étoile marquée <strong>①</strong> — elle brille en doré</li>
        <li>Suis les numéros <strong>dans l'ordre</strong>, une étoile à la fois</li>
        <li>Une ligne brillante se dessine entre chaque paire d'étoiles</li>
        <li>Complète toutes les étoiles pour révéler notre <strong>constellation en cœur</strong> ♥</li>
        <li>Touche <strong>↻ recommencer</strong> à tout moment pour repartir de zéro</li>
      </ul>
    `
  }
};
...
function showInstructions(level){
  ...
  if(!data) return;
  pendingLevel = level;
  $('#instrTag').textContent = `Niveau ${level}`;
...
const LOVE_LETTER = `Ma très chère [NAME], ma bunny chan,
...
/* ============== LocalStorage ============== */
function save(){
  // Disabled persistence so each reload starts completely fresh
}
function load(){
  try{
...
function l1Check(){
  ...
  const err = $('#l1Error');
  if(!v){ err.textContent = 'chuchote quelque chose pour moi ♥'; return; }
  const ok = ALLOWED_NAMES.some(a => v === a || v.includes(a));
  if(ok){
    playSfx('correct');
    err.style.color = '#a7f3d0';
    err.textContent = '✓ oui c\'est toi ♥';
    ...
    if(l1Tries >= 3){
      ...
      err.textContent = `réessaie (${3-l1Tries} essai${3-l1Tries === 1 ? '' : 's'} restant${3-l1Tries === 1 ? '' : 's'})`;
    }
...
async function initPuzzle(){
  ...
  const margin = 10;
  // Left side scatter
  for(let i=0;i<3;i++) slots.push({ x: margin, y: PUZZLE_OFFSET - 20 + i*160 });
  // Right side scatter (keep x within 600 viewBox limit)
  for(let i=0;i<3;i++) slots.push({ x: 430, y: PUZZLE_OFFSET - 20 + i*160 });
  // Top
  for(let i=0;i<2;i++) slots.push({ x: PUZZLE_OFFSET + 30 + i*180, y: margin });
  // Bottom
  for(let i=0;i<1;i++) slots.push({ x: PUZZLE_OFFSET + 120, y: 430 });
...
function startDrag(e, g){
  ...
  if(g.classList.contains('locked') || (piece && piece.placed)) return;
  e.preventDefault();
...
function solvePuzzle(){
  if (!puzzlePieces || !puzzlePieces.length) return;
...
const _l3SolveBtn = $('#l3SolveBtn');
if(_l3SolveBtn){
  _l3SolveBtn.addEventListener('click', () => { playSfx('click'); solvePuzzle(); });
}
...
function startRain(){
  ...
  if(headerEl) headerEl.innerHTML = `Attrape assez d'amour<br/>pour remplir mon cœur`;
...
function endRain(won){
  ...
  if(won || rainScore >= RAIN_TARGET){
    ...
    $('#btnL4Start').textContent = `Réessaie (${rainScore} ♥)`;
  }
}
...
function initConstellation(){
  ...
  $('#l5Hint').textContent = 'touche l\'étoile ① pour commencer ✦';
...
function tapStar(i){
  ...
  if(constelNext < CONSTEL_N){
    ...
    $('#l5Hint').textContent = `magnifique — touche maintenant l'étoile ${numCircle(constelNext+1)}`;
  } else {
...
function finishConstellation(){
  constelDone = true;
  $('#l5Hint').textContent = 'notre constellation est complète ♥';
...
/* =========================================================
   ...
   ========================================================= */
const LETTER_SIGN = 'Pour toujours et un jour de plus,\nrien qu\'à toi ♥';
...
function triggerSurprise(){
  ...
  $('#surpriseHello').textContent = `Pour toi, ${state.name || 'bunny chan'}`;
  startSparks();
  ...
  const text = LOVE_LETTER.replace('[NAME]', state.name || 'bunny chan');
  const el = $('#letterText');
...
function renderGallery(i, animate=true){
  ...
  img.onerror = () => { img.onerror = null; img.src = makePlaceholderDataURL(i+1, `SOUVENIR · 0${i+1}`); };
  img.src = item.src;
...
function makePlaceholderDataURL(idx, label){
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400">
    ...
    <text x="20" y="385" font-family="Inter,sans-serif" font-size="10" fill="rgba(255,255,255,0.7)">remplace par ta photo</text>
  </svg>`;
...
function makePuzzleDataURL(){
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600">
    ...
    <text x="30" y="50" font-family="Inter,sans-serif" font-size="14" font-weight="700" letter-spacing="3" fill="rgba(255,255,255,0.95)">PUZZLE · TOI &amp; MOI</text>
  </svg>`;
  return 'data:image/svg+xml;utf8,' + encodeURIComponent(svg);
}
...
function installImageFallbacks(){
  ...
  $$('.orbit-photo img').forEach((img, i) => {
    ...
    img.addEventListener('error', () => {
      img.src = makePlaceholderDataURL(idx, `SOUVENIR · 0${idx}`);
    });
  ...
  // Puzzle peek image
  const peek = $('#puzzlePreviewImg');
  if(peek){
    peek.addEventListener('error', () => { peek.src = makePuzzleDataURL(); });
    if(peek.complete && peek.naturalWidth === 0){ peek.src = makePuzzleDataURL(); }
    
    // Cheat/debug shortcut: double click target preview to auto-solve
    peek.addEventListener('dblclick', solvePuzzle);
  }
